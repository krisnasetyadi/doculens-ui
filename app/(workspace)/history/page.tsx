...
type AnySession = ChatSession | SessionSummary;

function getSessionId(s: AnySession) {
  return "id" in s ? s.id : s.session_id;
}
function getSessionTitle(s: AnySession) {
  return s.title;
}
function getUpdatedAt(s: AnySession) {
  return "updatedAt" in s ? s.updatedAt : s.updated_at;
}
function getMessageCount(s: AnySession) {
  return "messages" in s
    ? Math.floor(s.messages.length / 2)
    : Math.floor((s.message_count ?? 0) / 2);
}
function getPdfCollections(s: AnySession) {
  if ("session_id" in s) return s.pdf_collections ?? [];
  return s.pdfCollections ?? [];
}

function groupByDate(sessions: AnySession[]) {
  const map = new Map<string, AnySession[]>();
  for (const s of sessions) {
    const d = dayjs(getUpdatedAt(s));
    let label: string;
  ...
  const groups: { label: string; items: AnySession[] }[] = [];
...
export default function HistoryPage() {
  const { sessions: localSessions, deleteSession: deleteLocal, clearAllSessions } =
    useWorkspaceStore();
  const router = useRouter();
  const [search, setSearch] = useState("");
  const [confirmClear, setConfirmClear] = useState(false);
  const [backendSessions, setBackendSessions] = useState<SessionSummary[] | null>(null);
  const [loadingBackend, setLoadingBackend] = useState(true);

  const fetchBackend = () => {
    setLoadingBackend(true);
    SessionsApi.get<SessionSummary[]>()
      .then((data) => setBackendSessions(Array.isArray(data) ? data : []))
      .catch(() => setBackendSessions(null))
  ...
  const allSessions: AnySession[] = backendSessions ?? localSessions;

  const filtered = allSessions.filter((s) =>
    getSessionTitle(s).toLowerCase().includes(search.toLowerCase())
  );

  const grouped = groupByDate(
    [...filtered].sort(
      (a, b) =>
        new Date(getUpdatedAt(b)).getTime() - new Date(getUpdatedAt(a)).getTime()
    )
  );
  ...
  const handleDelete = (id: string) => {
    ...
    deleteLocal(id);
    if (backendSessions)
      setBackendSessions((prev) => prev?.filter((s) => s.session_id !== id) ?? null);
  };

  const handleClearAll = () => {
    allSessions.forEach((s) => SessionsApi.delete(getSessionId(s)).catch(() => {}));
    clearAllSessions();
    setBackendSessions([]);
    setConfirmClear(false);
  };

  const resumeSession = (s: AnySession) => {
    router.push(`/ask?session_id=${getSessionId(s)}`);
  };
      ...
      <div className="max-w-3xl mx-auto px-8 pt-10 pb-16">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <div className="flex items-center gap-2">
              <p className="font-['Inter'] text-muted-foreground text-sm">
                ...
                {backendSessions !== null ? " · synced" : " · local only"}
              </p>
              {backendSessions !== null && (
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <RefreshCw className={`h-4 w-4 ${loadingBackend ? "animate-spin" : ""}`} />
            </button>
        ...
        {/* Loading */}
        {loadingBackend && allSessions.length === 0 && (
        ...
        {/* No search results */}
        {allSessions.length > 0 && filtered.length === 0 && (
            ...
            <div key={label}>
              ...
              <div className="space-y-2">
                {items.map((session) => {
                  ...
                  const replyCount = getMessageCount(session);
                  const pdfCols = getPdfCollections(session);
                  return (
                    <div
                      key={sid}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-[10px] text-muted-foreground/50 font-['Inter']">
                            {dayjs(getUpdatedAt(session)).fromNow()}
                          </span>
