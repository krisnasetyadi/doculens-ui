...
> generating, or refactoring Vite SPA (React 18) codebases. Humans  
...
Comprehensive performance optimization guide for React applications (Vite SPA + React 18), designed for AI agents and LLMs. Contains rules across 7 categories, prioritized by impact from critical (eliminating waterfalls, reducing bundle size) to incremental (advanced patterns). Each rule includes detailed explanations, real-world examples comparing incorrect vs. correct implementations, and specific impact metrics to guide automated refactoring and code generation.

> **Scope:** This document covers client-side React only. Server-side (Next.js RSC, Server Actions, SWR) rules are not included.
...
1. [Eliminating Waterfalls](#1-eliminating-waterfalls) — **CRITICAL**
   ...
   - 1.3 [Parallelize Independent Async Operations](#13-parallelize-independent-async-operations)
...
5. [Rendering Performance](#5-rendering-performance) — **MEDIUM**
   ...
   - 5.7 [Use CSS Visibility for Show/Hide](#57-use-css-visibility-for-showhide)
   - 5.8 [Use Explicit Conditional Rendering](#58-use-explicit-conditional-rendering)
   - 5.9 [Use useTransition for Non-Urgent Updates](#59-use-usetransition-for-non-urgent-updates)
...
7. [Advanced Patterns](#7-advanced-patterns) — **LOW**
   ...
   - 7.3 [Stable Callback Refs to Avoid Stale Closures](#73-stable-callback-refs-to-avoid-stale-closures)
...
### 1.3 Parallelize Independent Async Operations

**Impact: CRITICAL (2-10× improvement)**

Start independent async operations immediately rather than sequentially, even if you don't await them yet.
...
**Alternative: Next.js 13.5+ only (N/A for this Vite SPA)**

```js
// next.config.js - use optimizePackageImports
// This option is Next.js-specific and not available in Vite.
// In this codebase, use direct imports as shown above.
module.exports = {
  experimental: {
...
The `typeof window !== 'undefined'` check ensures this import only executes in a browser context, preventing it from running during build-time module evaluation.
...
The `typeof window !== 'undefined'` check ensures the preload only executes in a browser context, preventing it from running during build-time module evaluation. It also delays loading until user intent or the relevant feature flag condition is met.
...
```tsx
// useKeyboardShortcut.ts — subscribes/unsubscribes, shares the singleton
...
// Multiple components can call this; only 1 listener is ever added to window
function Profile() {
...
> **Note:** Hydration mismatches and SSR breakage only occur in server-rendered frameworks (Next.js, Remix). This codebase is a Vite SPA with no SSR. The pattern below is included for reference but is **not applicable** here — there is no server rendering step to cause mismatches.

**Impact: MEDIUM (avoids visual flicker and hydration errors in SSR setups)**
...
### 5.7 Use CSS Visibility for Show/Hide

**Impact: MEDIUM (preserves state/DOM, avoids expensive remounts on repeated show/hide)**
...
**When to apply:** tabs where switching back is common, dropdowns/drawers with heavy content, charts that re-fetch on mount.

**When to stick with `isOpen ? <X /> : null`:** the component is cheap to remount, you need `useEffect` cleanup to run on hide, or the hidden content must not stay in the DOM for accessibility reasons.

> **Note:** `<Activity>` is a future React API (canary/experimental only, not available in React 18). Do not import it — use the CSS pattern above until it ships in a stable release.
...
### 5.9 Use useTransition for Non-Urgent State Updates

**Impact: LOW (reduces re-renders and improves perceived responsiveness)**
...
**When to use `useTransition`:** marking filter/tab/search input state updates as non-urgent; deferring expensive re-renders caused by a synchronous state change.

**When NOT to use `useTransition`:** tracking network requests in-flight (use TanStack Query `isLoading`/`isFetching`); wrapping `mutateAsync` or any async function (use `isPending` from `useMutation`).
...
### 7.3 Stable Callback Refs to Avoid Stale Closures
...
3. [https://vercel.com/blog/how-we-optimized-package-imports-in-next-js](https://vercel.com/blog/how-we-optimized-package-imports-in-next-js)
