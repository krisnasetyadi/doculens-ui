...
const MOCK_QUERY = "What's our Q3 reimbursement policy?";
...
const features = [
  {
    ...
    desc: "Understand and query everything in your files.",
  },
  {
    icon: "database",
    title: "Database Intelligence",
    desc: "Ask questions directly against your structured data.",
  },
  {
    icon: "chat_bubble",
    title: "Conversation Intelligence",
    desc: "Turn conversations into searchable institutional knowledge.",
  },
  {
    icon: "link",
    title: "Web Intelligence",
    desc: "Connect public web content to your knowledge base.",
  },
];
...
export function LandingFeatures() {
  ...
  // Timeline beats: [mock query] -> [one per source] -> [hub], each an equal share of scroll.
  const totalUnits = n + 2;
  const promptProgress = clamp01(progress * totalUnits);
  const localProgress = features.map((_, i) => clamp01((progress - (i + 1) / totalUnits) * totalUnits));
  ...
  return (
    <section id="features" className="max-w-5xl mx-auto px-6 py-20 w-full scroll-mt-20">
      ...
      <div
        ...
        style={reducedMotion ? undefined : { height: `${100 + totalUnits * STEP_VH}vh` }}
      >
        <div className={reducedMotion ? "" : "sticky top-0 h-screen flex flex-col items-center justify-center"}>
          <div className="w-full">
            <div
              className="flex items-center gap-3 rounded-full border border-border bg-card px-5 py-3 shadow-sm max-w-md mx-auto mb-16"
              style={{
                opacity: promptProgress,
                transform: `translateY(${(1 - promptProgress) * 10}px) scale(${0.92 + 0.08 * promptProgress})`,
              }}
            >
